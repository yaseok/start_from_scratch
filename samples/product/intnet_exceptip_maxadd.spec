# 내부 네트워크 및 예외IP 최대룰 추가
Tags: DEVICE.master_brk.ini, DEVICE.p_real_cent.ini
table:data/product/hips_int_net_except_ip_full.csv
...
* "VS_DUT01"에서 "policy_agent" 설정파일 "/var/policy/policy_agent.json" Agent 정책에 rule:"internal_network", name:<int_net_name>, ip:<int_ip>, description<int_net_desc>로 추가. xpath는 "body.policy_hips_agent.internal_network"이다
* "VS_DUT01"에서 "policy_agent" 설정파일 "/var/policy/policy_agent.json" Agent 정책에 rule:"except_ip", name:<except_name>, ip:<except_ip>, description<except_desc>로 추가. xpath는 "body.policy_hips_agent.except_ip"이다
