...
* "VS_DUT01"에서 "policy_agent" 정책 적용 CLI:"../exec.sh ./hipstest set policy agent /var/policy/policy_agent.json.enc"을 한다_예상결과 실패
...
* "VS_DUT01"에서 "policy_agent" 정책 암호화 CLI:"../exec.sh ./encrypt enc /var/policy/policy_agent.json"를 한다
* "VS_DUT01"에서 "policy_agent" 정책 적용 CLI:"../exec.sh ./hipstest set policy fw /var/policy/policy_agent.json.enc"을 한다
* "VS_DUT01"에서 DB:"../exec.sh ./sqlite3 /usr/local/ahnlab/chipsl/db/agent_policy_curr.db 'pragma key='test'; select * from config;'" 적용결과를 확인한다
...
* "ATTACK01"에서 hping으로 탐지 트래픽을 발생 한다 조건은 sip:<traffic_sip1>, dip:"$VS_DUT01.ip", sport:<traffic_sport>, dport:<traffic_dport>, protocol:<traffic_protocol> 이다
* "ATTACK01"에서 hping으로 탐지 트래픽을 발생 한다 조건은 sip:<traffic_sip2>, dip:"$VS_DUT01.ip", sport:<traffic_sport>, dport:<traffic_dport>, protocol:<traffic_protocol> 이다
* "ATTACK01"에서 hping으로 탐지 트래픽을 발생 한다 조건은 sip:<traffic_sip4>, dip:"$VS_DUT01.ip", sport:<traffic_sport>, dport:<traffic_dport>, protocol:<traffic_protocol> 이다
* 5초 동안 대기
* "VS_DUT01"에서 "event" 로그 처리결과를 확인 한다 예상결과가 <expect_result1>이고 로그결과는 <log_result1>이다, 추가판단조건 sip:<traffic_sip1>, dip:"$VS_DUT01.ip", block_reason:"sn", rule_name:"nation", block_nation:<user_nation1>, ndir:<ndir> json 사용
* "VS_DUT01"에서 "event" 로그 처리결과를 확인 한다 예상결과가 <expect_result2>이고 로그결과는 <log_result2>이다, 추가판단조건 sip:<traffic_sip2>, dip:"$VS_DUT01.ip", block_reason:"sn", rule_name:"nation", block_nation:<user_nation2>, ndir:<ndir> json 사용
* "VS_DUT01"에서 "event" 로그 처리결과를 확인 한다 예상결과가 <expect_result4>이고 로그결과는 <log_result4>이다, 추가판단조건 sip:<traffic_sip4>, dip:"$VS_DUT01.ip", block_reason:"sn", rule_name:"nation", block_nation:<user_nation4>, ndir:<ndir> json 사용
