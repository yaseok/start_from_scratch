...
* "VS_DUT01"에서 "policy_agent" 설정파일 "/var/policy/policy_agent.json" Agent 정책에 name:<intnet_name3>을 삭제. xpath는 "body.policy_hips_agent.internal_network.[*].name"이다
...
* "VS_DUT01"에서 "policy_agent" 설정파일 "/var/policy/policy_agent.json" Agent 정책에 rule:"internal_network", name:<intnet_name1>, ip:<internal_network1>, description<description>로 추가. xpath는 "body.policy_hips_agent.internal_network"이다
* "VS_DUT01"에서 "policy_agent" 정책 암호화 CLI:"../exec.sh ./encrypt enc /var/policy/policy_agent.json"를 한다
* "VS_DUT01"에서 "policy_agent" 정책 적용 CLI:"../exec.sh ./hipstest set policy agent /var/policy/policy_agent.json.enc"을 한다
* "VS_DUT01"에서 DB:"../exec.sh ./sqlite3 /usr/local/ahnlab/chipsl/db/agent_policy_curr.db 'pragma key='test'; select * from config;'" 적용결과를 확인한다

AGENT 정책 최대 추가된 상태에서 일부 룰 변경
* "VS_DUT01"에서 "policy_agent" 설정파일 "/var/policy/policy_agent.json" 정책 "name":<intnet_name2>을 ip:<internal_network2> 로 변경을 한다 xpath는 "body.policy_hips_agent.internal_network.[*].name" 이다
...
출발지 IP가 내부 네트워크에 포함되는 트래픽
traffic_sip1: 신규추가 내부네트워크 IP, traffic_sip2: 변경한 내부네트워크 IP, traffic_sip4: 기존에 있던 내부네트워크 IP
...
출발지 IP가 외부 네트워크에 해당되는 트래픽
* "ATTACK01"에서 FPG로 탐지 트래픽을 발생 한다 조건은 sip:<traffic_sip3>, dip:"$VS_DUT01.ip", sid:<sid>이다
* 5초 동안 대기
* "VS_DUT01"에서 "event" 로그 처리결과를 확인 한다 예상결과가 <expect_result1>이고 로그결과는 <log_result1>이다, 추가판단조건 sip:<traffic_sip1>, dip:"$VS_DUT01.ip", attackid:<sid>, action:<action>, blockaction:<block_action>, priority<severity>, ndir<ndir1> json 사용
* "VS_DUT01"에서 "event" 로그 처리결과를 확인 한다 예상결과가 <expect_result2>이고 로그결과는 <log_result2>이다, 추가판단조건 sip:<traffic_sip2>, dip:"$VS_DUT01.ip", attackid:<sid>, action:<action>, blockaction:<block_action>, priority<severity>, ndir<ndir2> json 사용
* "VS_DUT01"에서 "event" 로그 처리결과를 확인 한다 예상결과가 <expect_result3>이고 로그결과는 <log_result3>이다, 추가판단조건 sip:<traffic_sip3>, dip:"$VS_DUT01.ip", attackid:<sid>, action:<action>, blockaction:<block_action>, priority<severity>, ndir<ndir3> json 사용
* "VS_DUT01"에서 "event" 로그 처리결과를 확인 한다 예상결과가 <expect_result4>이고 로그결과는 <log_result4>이다, 추가판단조건 sip:<traffic_sip4>, dip:"$VS_DUT01.ip", attackid:<sid>, action:<action>, blockaction:<block_action>, priority<severity>, ndir<ndir4> json 사용
